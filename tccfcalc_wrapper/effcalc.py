...
def calculate_eff(nuclide, N, anal_fname, seed, is_json_input=False):
    ...
    if is_json_input:
        tcc_prepare = lib.get_tccfcalc_prepare_json()
        input_json_filename = os.path.join(cur_path, 'tccfcalc_input.json')
        error_num = tcc_prepare(bytes(input_json_filename, 'utf-8'), seed)
    else:
