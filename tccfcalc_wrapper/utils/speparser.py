...
class SpectrumReader:
    ...
    def parse_spe(spe_fname: str) -> Spectrum:
        ...
        with open(spe_fname, 'rb') as f:
            ...
            while parname != "SPECTR":
                parname, parvalue = SpectrumReader._readline(f)
                if parname == "SHIFR":
    ...
    @staticmethod
    def parse_txt(filename: str) -> Spectrum:
        spe_info = SpectrumInformation()
        with open(filename, 'r') as f:
            ...
            for line in f:
                line = line.strip()
                if '=' not in line:
                    raise Exception('Wrong txt format, awaits "=" in line: '+line)
                key, value = [w.strip() for w in line.split('=', maxsplit=1)]
                if key == 'SHIFR':
                    spe_info.name = value
                elif key == 'TLIVE':
                    spe_info.tlive = _str_to_float_def(value)
                elif key == 'TREAL':
                    spe_info.treal = _str_to_float_def(value)
                elif key == 'SPECTRTXT':
                    break
                else:
                    spe_info.headerdict[key] = value
            # spectrum
            spectrum_list = []
            for line in f:
                line = line.strip()
    ...
    @staticmethod
    def _readline(f) -> tp.Tuple[str, tp.Optional[str]]:
        c = ''
        param_name = ''
        param_value = ''
        is_has_value = False
...
def convert_spe2txt(spe_filename: str, txt_filename):
    save_spectrum_as_txt(
        SpectrumReader.parse_spe(spe_filename),
        txt_filename
    )
