...
def compare_physspec_output_data_with_errors(data1: tp.Dict[str, tp.Any], data2: tp.Dict[str, tp.Any]) -> bool:
    ...
    for i, (y1, y2) in enumerate(zip(calc_res1["y0"], calc_res2["y0"])):
        if not math.isclose(y1, y2):
            logging.warning(f"calculation results differs in y0 idx {i}: {y1} != {y2}")
            return False
    # x1
    for i, (x1, x2) in enumerate(zip(calc_res1["x1"], calc_res2["x1"])):
        if not math.isclose(x1, x2):
            logging.warning(f"calculation results differs in x1 idx {i}: {x1} != {x2}")
            return False
    # y1
    for i, (y1, dy1, y2, dy2) in enumerate(zip(calc_res1["y1"], calc_res1["dy1"], calc_res2["y1"], calc_res2["dy1"])):
        if not _compare_values_with_abs_k(y1, dy1, y2, dy2, _K):
            ...
            logging.warning(f"calculation results differs in y1 idx {i}: {y1} != {y2} with k_diff= {k_diff}")
            return False
    # x2
    for i, (x1, x2) in enumerate(zip(calc_res1["x2"], calc_res2["x2"])):
        if not math.isclose(x1, x2):
            logging.warning(f"calculation results differs in x2 idx {i}: {x1} != {x2}")
            return False
    # y2
    for i, (y1, y2) in enumerate(zip(calc_res1["y2"], calc_res2["y2"])):
        ...
        if not _compare_values_with_k(y1, dy1, y2, dy2, _K):
            k_diff = _k_diff(y1, dy1, y2, dy2)
            logging.warning(f"calculation results differs in y2 idx {i}: {y1} != {y2} with k_diff= {k_diff}")
            return False
    return True


def compare_physspec_output_with_errors(filename1: str, filename2: str, with_error: bool = False,
                                        skip_absent_keys: bool = False) -> bool:
    try:
        with open(filename1) as f1, open(filename2) as f2:
            ...
            if with_error:
                ...
                return compare_dicts(data1, data2, skip_absent_keys=skip_absent_keys)
