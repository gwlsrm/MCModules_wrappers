...
def calc_response_with(emin, emax, grid_points, grid_log, seed, histories):
    # energy grid
...
def calc_response_with(grid, seed, histories):
    ...
    error_num = response_prepare(bytes(input_filename, 'utf-8'), seed)
    if error_num:
    ...
    for energy in grid.grid:
        ...
        res = response_calculate(N, energy/1000,  DE)
